...
for i = 1:size(temp,3)-1
    temp(ix:end-ix+1, iz:end-iz+1, i) = gaussblur2d( H_parameters(ix:end-ix+1, iz:end-iz+1, i), x(ix:end-ix+1), z(iz:end-iz+1), usr_par.kernel.sigma.source );
end
temp(ix:end-ix+1, iz:end-iz+1, end) = gaussblur2d( H_parameters(ix:end-ix+1, iz:end-iz+1, end), x(ix:end-ix+1), z(iz:end-iz+1), usr_par.kernel.sigma.structure );
